...
//        $pattern = "/([\w-:\*]*)(?:\#([\w-]+)|\.([\w-]+))?(?:\[@?(!?[\w-]+)(?:([!*^$]?=)[\"']?(.*?)[\"']?)?\])?([\/, ]+)/is";
    ...
    function convert_text($text)
    {
        ...
        if (!empty($sourceCharset) && !empty($targetCharset) && (strcasecmp($sourceCharset, $targetCharset) != 0))
        {
            ...
            {
                $converted_text =  utf8_encode ( $text);//iconv($sourceCharset, $targetCharset, $text);
            }
        }
...
class simple_html_dom
{
    ...
    {
        $args = func_get_args();
        $this->load(call_user_func_array('curl_get_contents', $args), true);
...
function curl_get_contents($url)
{
    ...
    curl_setopt($ch,CURLOPT_USERAGENT,'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Ubuntu Chromium/67.0.3396.99 Chrome/67.0.3396.99 Safari/537.36');
    
    curl_setopt($ch,CURLOPT_COOKIE,'_ga=GA1.3.1114978351.1532251470; has_js=1; SSESS2720f7635bdba1f946e9361ac22d22e2=MkH-RWYUOSNDYwD1Kv3LHHkk23xIgbowqsiVy5p1S64; _gid=GA1.3.472758833.1532847652; Drupal.tableDrag.showWeight=0');
    $html = curl_exec($ch);
    $data = curl_exec($ch);
    curl_close($ch);
    return $data;
